...
\proof
Since $d_c([1,2,3,\ldots,n], [n-1,n,n-2,n-3, \ldots, 1]) = \frac{(n-1)(n-2)}{2}$, the claim follows directly from from Theorem~\ref{thm:caterpillar_convex} and Lemma~\ref{lemma:diameter_bound}.
\endproof
...
\begin{proof}
Recall that with Theorem~\ref{thm:diameter} we know that $\Delta(\rnni) = \frac{(n-1)(n-2)}{2}$.
...
By this we obtain Theorem~\ref{thm:radius}.
