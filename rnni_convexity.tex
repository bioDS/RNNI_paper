...
When analysing the geometry of $\rnni$, we will consider a subset of trees and see that it is \emph{convex} in $\rnni$, which means that every pair of trees from this subset is connected by a shortest path that stays within this subset.
...
For example, \textcite{Whidden2016-kl} were able to achieve up to seven taxa in the much smaller $\spr$ space, when it required considering pairs of trees.
...
We have computationally shown that the algorithm $\findpath$ finds the correct distance for trees up to six taxa \autocite{Collienne2019}.
...
Every pair of caterpillar trees $T, R$ in $\rnni$ are connected by a caterpillar path of length $d(T,R)$, that is, the set of caterpillar trees is convex in $\rnni$ (Theorem~\ref{thm:caterpillar_convex}).
...
To prove the reverse, assume that $d_c(T,R) = \frac{(n-1)(n-2)}{2}$.
