...
\begin{lemma}
$\csort(T, R)$ returns a shortest caterpillar path between $T$ and $R$.
\end{lemma}

\proof
Let $p$ be an arbitrary caterpillar path.
Every $\rnni$ move along $p$ can decrease the number of inversions of taxa in $T'$ and $R$ by either $1$ or $2$.
A decrease by $2$ is only possible when $T' = [x, y, z, \ldots]$, $z<\min(x,y)$, and the tree after the move is $[z, \min(x, y), \max(x, y), \ldots]$.
\todo{Explain why more moves that fix two inversion each are not possible.}
Hence $p$ is a shortest caterpillar path if every move along $p$ either is of this type or decreases the number of inversions by one.
Note that $\csort(T, R)$ satisfies this property.
\endproof
...
\begin{proof}
We prove this theorem by backward induction on the length of a shortest caterpillar path $d_c(T,R) = l$ ($l$ is the induction parameter) between trees $T$ and $R$.
The backward induction basis follows by Lemma~\ref{lemma:caterpillar_dist=diameter}, as the maximum possible value for $l$ is $\frac{(n-1)(n-2)}{2}$.

Let $T$ and $R$ be caterpillar trees such that $d_c(T, R) = l < \frac{(n-1)(n-2)}{2}$.
Since $d_c(T, R) < \frac{(n-1)(n-2)}{2}$, there must be a step in $\csort(T, R)$ where the taxon that is moved up is not part of the cherry (of $T'$) at the beginning of this step.
...
\todo{No.
As $x$ and $y$ don't have to be neighbours in $T$.}
$d(T, \hat T) = 1$ and $d_c(\hat T,R) = d_c(T,R) + 1 = l + 1$.
