...
Let us now introduce a representation of caterpillar trees as lists of taxa, which is a simplification of the cluster representation for caterpillar trees and works as follows.
In the list representing a caterpillar tree its taxa are ordered non-decreasing, according to the ranks of their parents.
Note that the only two elements in such a list that have parents of equal rank are the first two elements $l_1$ and $l_2$.
As we assume to have taxon set $X = \{1, \ldots, n\}$, we assume that they are order in the list such that $l_1 < l_2$, according to their ordering as natural numbers.
...
Let $T$ and $R$ be the input trees given by their list representations $L_T$ and $L_R$ as introduced above.
The output path $p$ is built stepwise by adding a new list to the end of $p$.
