...
Besides proving some geometrical properties of $\rnni$, we also introduce some algorithms in this paper.
Our work highlights the importance of these algorithms as they are important for proving geometric properties such as diameter and radius of the $\rnni$ graph.
The most important algorithm we introduced might be $\findpath$, which computes exact distances for all pairs of trees on up to six taxa.
...
This fact that computing $\rnni$ distances between caterpillar trees only requires polynomial running time already suggests that there is a difference in complexity of computing distances in $\rnni$ and $\nni$.
In Section~\ref{section:cluster_theorem} we supported this claim further by discussing the proof of $\np$-completeness of computing distances in $\nni$ and showing that this proof does not work for $\rnni$.
Specifically, we claimed the Cluster Theorem (Conjetcure~\ref{conjecture:cluster_theorem}), which does not hold in $\nni$, but does hold in the $\rnni$ for trees on up to six taxa \autocite{Collienne2019}.
Though the Cluster Theorem remains a conjecture, the results of this paper suggest that it is true.


\subsection{Open Problems}

In Section~\ref{section:cluster_theorem} we claim that the Cluster Theorem holds for $\rnni$.
