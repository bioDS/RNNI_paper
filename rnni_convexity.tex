...
We have implemented all these algorithms in Python and the source code is available at \autocite{Collienne2019}.
...
Specifically, in step $k$ ($k = 0, \ldots, n-3$) the parent of taxon $x_{n-k}$ is moved up by $\nni$ moves to the position it has in $R$.
...
We will give a counterexample to the Split Theorem in $\rnni$ and consider a variant of the Weak Split Theorem that states that every shared split of taxa is maintained along a shortest path.
...
\begin{proof}
First note that $d(T,R) = \frac{(n-1)(n-2)}{2}$ implies $d_c(T,R) = \frac{(n-1)(n-2)}{2}$.Indeed, in each step $i=0, \ldots, n-3$ of $\csort$ at most $n-2-i$ pairs of taxa swap positions, as in the worst case the taxon considered in step $k$ is moved from the cherry of the tree up to the position $n-k$.
...
We prove that $d(T,R) = \frac{(n-1)(n-2)}{2}$ by induction on the number of taxa $n$. The induction basis for $n=3$ taxa is trivial as the $\rnni$ graph is a triangle on three caterpillar trees.

For the induction step we assume without loss of generality that $T$ is the caterpillar tree $[1,2,3,\ldots,n+1]$ and $R$ is a caterpillar tree at distance $\frac{n(n-1)}{2}$ to $T$.
