...
For the induction step we assume without loss of generality that $T$ is the caterpillar tree $[1,2,3,\ldots,n+1]$ and $R$ is a caterpillar tree
%TODO: this is an incorrect assumption -- we need to prove this!
at distance $\frac{n(n-1)}{2}$ from $T$.
Consider the path $p = \csort(T, R)$ of length
Note that in this case taxon $n+1$ has to be in the cherry of $R$.
Otherwise, $\csort$ would find a path between $T$ and $R$ shorter than $\frac{n(n-1)}{2}$, which can be shown as in (\ref{csort_max}) above.
