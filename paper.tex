...
We will prove that a tree $R = \mdtree(T)$ has distance $d(T,R) = \Delta(\rnni)$ to $T$ if $T$ is a caterpillar tree, after introducing the algorithm.
...
In order to find the radius of the $\rnni$ graph, the algorithm $\mdtree$ has been introduced in Section~\ref{section:diameter}.
