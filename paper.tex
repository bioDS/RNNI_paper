...
In Conjecture~\ref{conjecture:radius} we claim that the radius of $\rnni$ equals its diameter.
Algorithm~\ref{alg:max_dist_tree} suggests this, since it computes for an arbitrary input tree $T$ a caterpillar tree $R$ with distance $d(T,R) = \frac{(n-1)(n-2)}{2}$ for $n \leq k$. \todo{$k = ?$, check computationally (or prove this -> see conjecture below)}

\begin{conjecture}
    It is $rad(\rnni) = \Delta(\rnni) = \frac{(n-1)(n-2)}{2}$.
    \label{conjecture:radius}
\end{conjecture}
...
\begin{algorithmic}[1]
	...
	\FOR {taxon $a$ in $L$}
		\STATE Add an internal node as parent of $a$ on an edge in $R$ such that the new internal node has rank one less than the previously lowest internal node and $R$ is a caterpillar tree
	\ENDFOR
	\RETURN $R$
\end{algorithmic}
\end{algorithm}
...
\begin{conjecture}
    Let $T$ be an arbitrary tree and $R$ a caterpillar tree.
    ...
    \label{conjecture:max_dist_caterpillar}
\end{conjecture}

%TODO
%prove this conjecture! (<= would be sufficient for the conjecture above!)
