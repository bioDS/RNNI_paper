...
Before introducing the idea of this algorithm, we need further definitions.
...
\emph{Trivial clusters} are clusters induced by leaves and the root of a tree.
...
This list $C_1, \ldots, C_{n-1}$ of clusters, which is ordered increasingly according to the ranks of their corresponding nodes, uniquely defines $R$.
...
Update $\hat{T}$ by decreasing the rank of $\mrca_{\hat{T}}(C_i)$ with an $\rnni$ move until it has the same rank $i$ as in $R$. \todo{Do we need to explain why there is always an $\rnni$ move that moves the $\mrca$ down?}
...
This algorithm gives an upper bound for the diameter of $\rnni$, which improves the one given in \autocite{Gavryushkin2018-ol}.
...
\begin{lemma}
    $\Delta(\rnni) \leq \frac{(n-1)(n-2)}{2}$, where $\Delta(G) = \max\limits_{u,v \in V(G)}d(u,v)$ denotes the diameter of graph $G$.
...
We implemented this algorithm and could produce the graph for trees with up to $8$ taxa. \todo{Shall we say more about the algorithm here?}
This enables us to compute exact distances between trees of this size.
In particular, this enables us to compare exact distances with distances we receive from algorithms introduced in previous sections and to verify the Cluster Theorem, which was raised in Section~\ref{section:split_theorem}, for small trees.
...
Following this observation, we claim following conjecture:
...
Besides the diameter, the \emph{radius} of a graph $G$, which is defined by $rad(G) =  \min\limits_u \max\limits_v d(u,v)$ is a further interesting property of a graph.
We will use Algorithm~\ref{alg:max_dist_tree} to show that in $\rnni$ the radius equals the diameter.

%TODO
%Explain the following algorithm, not only Pseudo-Code!
...
\begin{lemma}
    ...
    A tree $R$ that is computed by applying Algorithm~\ref{alg:max_dist_tree} on $R$ has distance $d(T,R) = \Delta(\rnni)$ to $T$.
...
\begin{proof}
    ...
    Induction basis: $n = 3$\\
    Algorithm~\ref{alg:max_dist_tree} computes either $R = ((1,3),2)$ or $R'= ((2,3),1)$ when applied to $T = ((1,2),3)$.
    Since it is $d(T,R) = d(T,R') = 1 = \Delta(\rnni_3)$, the lemma is true for trees on $n=3$ taxa.

    Induction step: $n \to n+1$\\
    Induction hypothesis: For a caterpillar tree $T$ on $n$ taxa Algorithm~\ref{alg:max_dist_tree} computes a tree $R$ with distance $d(T,R) = \frac{(n-1)(n-2)}{2}$.\\
    Let us now  assume that $T$ is a tree on $n+1$ taxa.
    In the list $L$ produced in Algorithm~\ref{alg:max_dist_tree}, when applied to $T$, taxon $n+1$ is the last one.
    ...
    It is easy to see that $R|_n$ can be received from $T|_n$ by applying Algorithm~\ref{alg:max_dist_tree}.
...
In Algorithm~\ref{alg:max_dist_tree} it is not uniquely determined, where a new taxon is added to the already existing tree.
...
This is the reason why one can receive every tree topology on $n$ taxa when applying Algorithm~\ref{alg:max_dist_tree} onto a caterpillar tree.
And because permuting the labels of the trees does not change the distance, there is for every tree $R$ on $n$ taxa a caterpillar tree $T$ with distance $d(T,R) = \Delta(\rnni) = \frac{(n-1)(n-2)}{2}$.
