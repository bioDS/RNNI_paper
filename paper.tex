...
\begin{conjecture}
    Algorithm~\ref{alg:max_dist_tree} computes a tree with maximum distance $\Delta(\rnniu)$ from a given tree $T$.
\end{conjecture}

According to Algorithm~\ref{alg:max_dist_tree}, the number of trees with maximum distance to a given tree $T$ depends on the number of cherries of $T$.
...
\begin{algorithmic}[1]
	\STATE $R$ only contains cherry taxa of $T$ and has internal node with rank $n-1$
    \STATE In list $L$ all taxa $\{1,\ldots,n\}$ are sorted such that the ranks of their parents in $T$ do not decrease
	\FOR {taxon $a$ in $L$}
