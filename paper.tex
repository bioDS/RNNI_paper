...
\begin{lemma}
    ...
    $T$ and $R$ have distance $d(T,R) = \Delta(\rnniu) = \frac{(n-1)(n-2)}{2}$ if, and only if, they have caterpillar distance $d_c(T,R) = \Delta(\rnniu) = \frac{(n-1)(n-2)}{2}$. \todo{define $d_c$}
    \label{lemma:caterpillar_dist=diameter}
\end{lemma}

    % \todo{At the top of $R$ it could also be $2,1$, or $t=1$}
    %
    % \begin{figure}[H]
    %     \centering
    %     \includegraphics[width=0.6\textwidth]{caterpillar_max_dist}
    %     \caption{Caterpillar trees $T$ and $R$ with distance $d(T,R) = \Delta(\rnniu)$}
    %     \label{fig:caterpillar_max_distance}
    % \end{figure}

\begin{proof}
    Let $d_c(T,R)$ denote the length of a shortest caterpillar path from $T$ to $R$, that is a caterpillar path that is shortest among all caterpillar paths from $T$ to $R$.
    We need to prove that $d(T,R) = \Delta(\rnniu)$ if, and only if, $d_c(T,R) = \Delta(\rnniu)$.

    Let $T$ and $R$ have distance $d(T,R) = \Delta(\rnniu)$.
    We can compute a  shortest caterpillar path from $T$ to $R$ using Bubble Sort.
    ...
    Let now $T$ and $R$ have caterpillar distance $d_c(T,R) = \Delta(\rnniu)$.
    We prove $d(T,R) = d_c(T,R)$ by induction on the number of taxa $n$ of $T$ and $R$:
    We assume without loss of generality that $T$ is the caterpillar tree $(\ldots ((1,2),3), \ldots, n)$.
    Otherwise, we re-label $T$ and $R$.
    Let $R$ be a caterpillar tree with distance $\Delta(\rnniu)$ to $T$.
    ...
    Let $T, R$ be trees on $n+1$ taxa with caterpillar distance $\Delta(\rnniu_{n+1})$.
    When applying the Bubble Sort algorithm to compute a path between $R$ and $T$, each pair of taxa that is compared in that algorithm exchanges, since there must be $\Delta(\rnniu)=\frac{(n-1)(n-2)}{2}$ moves between $T$ and $R$.
    So we can conclude that taxon $n+1$ must be in the cherry of $R$.
    Let us consider the trees $T|_n$ to $R|_n$ that result from $T$ and $R$ by deleting taxon $n+1$.
    When running Bubble Sort to compute a path from $T|_n$ to $R|_n$ one can observe the same exchanges of taxa as on the Bubble Sort path from $T$ to $R$, except for the $n-1$ ones that involve taxon $n+1$.
    ...
    With Lemma~\ref{lemma:distance_delete_taxon} we know that the distance between $T|_n$ and $R|_n$ is at least $d(T|_n, R|_n) \leq d(T,R) - |r_{T}(parent(n+1}) - r_{R}(parent(n+1})|$.
    We also know $|r_{T}(parent(n+1}) - r_{R}(parent(n+1})| = n-1$
    If we assume that there is a path from $T$ to $R$ of length less than $\Delta(\rnniu_{n+1}) = \frac{n(n-1)}{2}$, it would follow that $d(T|_n, R|_n) \leq d(T,R) - |r_{T}(parent(n+1}) - r_{R}(parent(n+1})| < \frac{n(n-1)}{2} - (n-1) < \frac{(n-1)(n-2)}{2}$.
    So it would be $d(T|_n, R|_n) < \Delta(\rnniu_n)$.
    Since this is a contradiction to $d(T|_n, R|_n) = \Delta(\rnniu_n)$, it must be $d(T,R) = \Delta(\rnniu_{n+1}) = \frac{n(n-1)}{2} $.
...
\begin{proof}
    ...
    With Lemma~\ref{lemma:caterpillar_dist=diameter} one can see that for each pair of caterpillar trees $T$, $R$ with caterpillar distance $d_c(T,R) = \Delta(\rnniu)$, it is $d(T,R) = d_c(T,R)$, which means that there is a shortest path between $T$ and $R$ that is a caterpillar path.
    ...
    Since $l < \Delta(\rnniu) = \frac{(n-1)(n-2)}{2}$, there is one loop $k$ in the Bubble Sort algorithm from $R$ to $T$ where there are less than $n-2-k$ swaps of neighboured taxa.
    ...
    If we assume that loop $k$ is the first loop where a single taxon is not moved by $n-k-2$ $\rnni$ moves, it follows that $a$ and $b$ must be neighboured taxa in $R$.
    Let $R'$ be the caterpillar tree resulting from $R$ by exchanging $a$ and $b$.
    It follows that $R$ has caterpillar distance $d_c(T,R') = d_c(T,R) + 1 = l+1$ to $T$ and $d(R,R') = 1$.
    We can apply the induction hypothesis on $T$ and $R'$ and know that $d(T,R') = l+1$.
    If it was $d(T,R) < l$, it would hold $d(T,R') \leq d(T,R) + d(R,R') < l + 1$ which contradicts the induction hypothesis.
    We can conclude that $d(T,R) = l$.
...
\begin{theorem}
	There are trees $T,R$ in $\nni$ sharing a partition which is not shared by any intermediate tree on a shortest path from $T$ to $R$.
	\label{thm:split_nni}
\end{theorem}
...
Let $T$ and $R$ be two caterpillar trees.
If each taxon $a$ is replaced by a cherry $(a_1,a_2)$ in both trees $T$ and $R$, the resulting trees $R$ and $R'$ have distance $d_{\nni}(R,R') = d_{\nni}(T,R)$ in the $\nni$ graph.
...
For the given example there is a shortest path in $\nni$ where at first $T$ is transformed to a tree $R$ with $n$ cherries where each cherry contains one taxon of $\{1, \ldots, n\}$ and one taxon of $\{n+1, \ldots, 2n\}$.
Afterwards, $R$ is tranformed to a tree $R'$ that contains the same cherries as $R$.
