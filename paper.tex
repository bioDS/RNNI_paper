...
\todo{LC: make sure that we move all definitions in this paper to the right place}

Within this section we will first two introduce algorithms, namely $\findpath$ and $\csort$, for computing paths between two given trees in $\rnni$.
We will use them later to prove some geometric properties of the $\rnni$ graph in section~\ref{section:geometry}.
Additionally we will present the algorithm $\mdtree$, which computes a caterpillar tree with maximum distance
\todo{LC: can we use the term max dist here?}
from a given tree.
...
\todo{LC: Introducing words}

\todo{LC: move the following definition? - it's actually quite important not only for the algorithm, so we might want to have it in the introduction?}
...
\todo{Check that the indexing of $C_i$ is still correct. ($i=1,\ldots,n-2$)}
non-trivial clusters of a tree, sorted according to the rank of the corresponding node, defines the tree unambiguously.
...
\todo{LC: so we ever use trivial clusters? If so, we need to add a comment here}

We are now presenting an algorithm for computing paths in $\rnni$ between any two given trees.
...
Within every step of the algorithm we add one more tree to extend a path $p$, which will be the output path of $\findpath$.
First, set $\hat{T} = T$ to be the first element in $p$.
With $\hat T$ we denote the last tree of $p$ at each step in the algorithm.
Consider the clusters $C_1, \ldots, C_{n-2}$ of $R$ in this order.
When arriving at cluster $C_k$, find the node inducing the smallest cluster containing all elements of $C_k$ in $\hat T$.
This node is denoted by $\mrca_{\hat{T}}(C_k)$ (short for \emph{most recent common ancestor}).
\todo{LC: make sure you don't use mrca before this!}
...
With Lemma~\ref{lemma:mrca_move} we know that there is always exactly one $\rnni$ move possible that decreases the rank of $\mrca_{\hat{T}}(C_i)$.
Repeat this until $\mrca_{\hat{T}}(C_i)$ has the same rank $i$ in $\hat T$ as in $R$.
...
With $\findpath$ we can compute a path between two arbitrary trees in $\rnni$.
The length of such a path can then be used as approximation for the distance between the trees.
We implemented $\findpath$ in Python.
\todo{LC: cite, link to github code?}
\todo{LC: Shall we mention the exact algorithm at the beginning of this section already?}
In~\autocite{Gavryushkin2018-ol} an algorithm for computing the $\rnni$ graph is being introduced.
We implemented this algorithm
\todo{LC: github?}
and used
\todo{LC: which algorithms did we use?}
to compute exact distances between all pairs of trees on up to $7$ taxa.
\todo{LC: Is 7 the actual number?}
By comparing these distances with the distances given by $\findpath$, we find that $\findpath$ gives the correct distances for all pairs of trees on up to $7$ taxa.
\todo{LS: Is it 7? How did we compute all distances with $\findpath$? - We used the symmetry of $\rnni$}
...
In Section~\ref{section:diameter} we will see that for any pair of trees, $\findpath$ computes a path that is at most as long as the the diameter of $\rnni$.
Thus, if the distance between two trees equals the diameter, $\findpath$ computes the correct distance.
\todo{LC: Can we say something about the approximation ratio here?}


\subsection{$\csort$}
\label{section:alg_cpath}

\todo{LC: move definitions of caterpillar stuff here}
\todo{LC: pseudo code}

Before we start introducing an algorithm for computing paths between caterpillar trees, we need to introduce further notations.
...
\todo{LC: structure in the following paragraph!}
We next introduce a unique way to represent of caterpillar trees as sequences of taxa.
We will use this to show how one can use the classical \emph{Bubble Sort} algorithm \autocite{Knuth1997-pi} to compute caterpillar paths.
This algorithm is denoted by $\csort$ and the pseudo-code is provided in Algorithm.
\todo{LC: pseudo-code}
One can see caterpillar trees as sequences of taxa, ordered non-decreasing according to the ranks of their parents.
Note that the only two elements in such a sequence that have parents of equal rank are the first two elements, the cherry taxa $c_1$ and $c_2$.
To make the sequence representation of caterpillar trees unique, we assume that for it is $c_1 < c_2$.
...
The only exception are the two first three elements in the sequence.
Corresponding to $\nni$ moves on the lowest internal edge are exchanges of either $c_1$ or $c_2$ with the third elements $c_3$ in the sequence.
But a swap of $c_1$ and $c_2$ does not correspond to any move in $\rnni$.
Therefore, we need to adapt Bubble Sort for the first three elements of the sequence as follows:
\todo{LC: add the pseudocode and refer to the line where $\csort$ differs from Bubble Sort! (We assume that we are at the point where we do some changes only on the first positions of the sequence)}
If the rank of $\parent_R(c_3)$ is higher than the ranks of both $\parent_R(c_1)$ and $\parent_R(c_2)$ in $R$, there are no swaps needed for these taxa.
If just one of the two taxa $c_1$ or $c_2$ is on the other side of $c_3$ in $R$, this taxon swaps position with $c_3$ in the sequence.
And if both $c_1$ and $c_2$ are on the other side of $c_3$ in $R$, the taxon that has the parent with higher rank in $R$ swaps with $c_3$.
This makes sure that there is no additional swap of $c_1$ and $c_2$ needed on the subsequent path to $R$.
