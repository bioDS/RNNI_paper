...
A \amph{rooted binary phylogenetic tree} a set $X$ is a pair $(t, \Phi)$ where $t$ is a rooted binary tree with leaf set $\mathcal{L}(t)$ and $\Phi:X \mapsto \mathcal{L}(T)$ a bijective mapping. \todo{do we need to define leaves and internal nodes, child, parent?}
...
The set of taxa that are descending from an internal edge $e$ is called \emph{cluster}. \todo{OK?}
...
Given a subset $M$ of the set of taxa $X$, the \emph{most recent common ancestor} $mrca(M)$ of $M$ is the internal node with lowest rank that is ancestor of all taxa in $M$. \todo{OK?}
...
% With the knowledge that the set of caterpillar trees is convex in $\rnniu$, we can prove the Cluster Theorem restricted to caterpillar trees.

\begin{conjecture}
    Let $T_1$ and $T_2$ be caterpillar trees.
    If $T_1$ and $T_2$ contain the same cluster $C$, then $C$ is present as cluster in every tree on every shortest path between $T_1$ and $T_2$. \todo{Cluster means non-trivial cluster -> def chapter?}
\end{conjecture}
