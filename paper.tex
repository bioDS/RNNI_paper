...
In \autocite{Gavryushkin2018-ol} an exact algorithm for computing the $\rnni$ graph has been introduced.
We implemented this algorithm and used it for computing this graph for trees on up to $7$ taxa.
\todo{LC: $7$ or $8$?}
...
\todo{LC: move this up (before lemma1?)}
...
The running time of $\findpath$, implemented as described by the pseudocode in Algorithm~\ref{alg:find_path}, is quadratic in the number of taxa.
...
For a caterpillar tree $T = [C_1, \ldots, C_{n-2}]$
\todo{LC: Do we need to mention that we refer to a tree by this notation, or do we need to say that this is the cluster representation of a tree?}
it is $C_i \subset C_{i+1}$ with $|C_i| = |C_{i+1}|$ for all $i = 1, \ldots, n-3$.
...
This is a well known property of Bubble Sort that $\csort$ inherits.
\todo{LC: cite}
...
The worst-case execution of $\findpath$ would require each of the $n-2$ clusters $C_i$, considered in Line~\ref{alg:find_path:line:cluster} of Algorithm~\ref{alg:find_path}, to be moved by $n-1-i$ $\rnni$ operations in $\hat{T}$.
...
So we can follow that the rank of $\parent_T(n+1)$ can be decreased by at most one by an $\rnni$ move on $T$.
...
In fact, every tree topology on $n$ taxa can be resulting from $\mdtree$, since there is no restriction on where to add the new taxon in the already existing tree in Line~\ref{alg:mdtree_lineAddTaxon} of Algorithm~\ref{alg:max_dist_tree}.
