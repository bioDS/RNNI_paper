...
For a tree $T$, a pair of taxa adjacent to a common internal node called a \emph{cherry}.
...
In particular, for a single taxon $x \in X$, $\mrca(\{x\})$ is the \emph{parent} of $x$ and will be denoted by $\parent(x)$.
...
\todo{This text comes out of the blue.
Generally, before going into explanations like this it's a good idea to say what's ahead.}
...
\todo{This should have been stated before, where you say what is going on.}
Moreover, it gives an upper bound for the diameter of $\rnni$, which improves the one given in \autocite{Gavryushkin2017}:

\begin{lemma}
%TODO: here and generally, I prefer introducing definitions like this as you go but it's the matter of preference so up to you.
...
\begin{proof}
    Let $p$ be a shortest path from tree $T$ to $R$ and $p|_n$ the path resulting from deleting taxon $n+1$ from all trees on $p$.
    This new path $p|_n$ is then a path from $T$ to $R$.
    Showing that at least $\delta := |\rank_T(\parent(n+1)) - \rank_R(\parent(n+1))|$ $\rnni$ moves are deleted from $p$ to receive $p|_n$ results in $d(T|_n,R|_n) \leq |p|_n| \leq |p| - \delta = d(T,R) -\delta$ which proves the lemma.
    ...
    Applying this to $\parent(n+1)$ and knowing that the rank of this node differs by $\delta$ between $T$ and $R$ proves that $d(T|_n, R|_n) \leq d(T,R) - \delta$.
