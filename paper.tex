...
These will be used later to prove some geometric properties of the $\rnni$ graph in section~\ref{section:geometry}.
...
\todo{Check that the indexing of $C_i$ is still correct.}
...
To compute a path from tree $T$ to $R$ we consider clusters $C_1, \ldots, C_{n-2}$ of $R$ (the destination tree) where $C_i$ is induced by the node with rank $i$ for $i = 1, \ldots, n-2$.
...
Since we consider the clusters of $R$ such that the ranks of the corresponding internal nodes in $R$ increase, $\mrca_{\hat{T}}(C_i)$ is the node inducing cluster $C_i$ in $\hat{T}$ after these $\rnni$ moves.
...
\todo{LC: Is it worth introducing intervals for $\rnni$ moves for the proof of the following lemma?}
...
\todo{LC: Change the references to sections}
