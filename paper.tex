...
\begin{proof}
    ...
    This means that the taxon $a$ that has parent with rank $n-2-k$ in $T'$ does not need to move all the way up starting at the cherry.
    Hence, there is a taxon $b$ that has parent with rank one less than the parent of taxon $a$ in $T'$ that does not exchange with $a$ on the path to $p$. \todo{$a$ and $b$ do not need to be neighbours in $T'$!}
    Let $T''$ be the caterpillar tree resulting from $T'$ by exchanging $a$ and $b$.
    It follows that $T'$ is on a shortest caterpillar path from $T''$ to $T$ and therefore, $d_c(T,T'') = d_c(T,T') + 1 = l+1$.
