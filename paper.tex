...
We next introduce a representation of caterpillar trees as sequences of taxa.
...
\todo{LC: pseudo-code}
Ordering the taxa of a caterpillar trees as a non-decreasing, according to the ranks of their parents, gives us a sequence of length $n$.
Note that the only two elements in such a sequence that have parents of equal rank are the first two elements $c_1$ and $c_2$.
...
To make the sequence representation of caterpillar trees unique, we assume that it is $c_1 < c_2$ for the first two elements in the sequence.

In the following we explain how $\rnni$ moves between caterpillar trees work in the sequence representation.
...
$\nni$ moves between caterpillar trees correspond to swaps of two neighboured elements in the sequence representation.
...
Corresponding to the $\nni$ moves on the lowest internal edge are exchanges of either the first or the second element with the third elements in the sequence representation.
